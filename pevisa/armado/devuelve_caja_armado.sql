...
begin
  ...
  for r in cr_cajas loop
    ...
    dbms_output.put_line('1.. CODIGO_CAJA: ' || r.codigo_caja || '  procesados...: ' || sql%rowcount);
    --
--
    ...
    select p.numero_oa, 2, sysdate, 'ORDEN REGRESADA A PRODUCCION A PEDIDO DE DAVID', user, null
      from produccion_armado p, produccion_armado_log l
     where p.numero_oa = l.numero_oa and l.cod_caja = (r.codigo_caja);
    dbms_output.put_line('2.. CODIGO_CAJA: ' || r.codigo_caja || '  procesados...: ' || sql%rowcount);
    --
--
    ...
    dbms_output.put_line('3.. CODIGO_CAJA: ' || r.codigo_caja || '  procesados...: ' || sql%rowcount);
    --
--
    ...
    dbms_output.put_line('4.. CODIGO_CAJA: ' || r.codigo_caja || '  procesados...: ' || sql%rowcount);
    --
--
    ---------------------------------------------------- 5. Elimina el detalle de la caja
    delete produccion_armado_cajas_det
     where cod_caja = (r.codigo_caja);
    dbms_output.put_line('5.. CODIGO_CAJA: ' || r.codigo_caja || '  procesados...: ' || sql%rowcount);
    --
--
    ------------------------------------------------------ 6. Elimina la revisión de la caja
    delete produccion_armado_rev
     where cod_caja = (r.codigo_caja);
    dbms_output.put_line('6.. CODIGO_CAJA: ' || r.codigo_caja || '  procesados...: ' || sql%rowcount);
    --
--
    ...
    update pr_ot
     ...
     where numero in (
       ...
       and estado = 6;
    dbms_output.put_line('7.. CODIGO_CAJA: ' || r.codigo_caja || '  procesados...: ' || sql%rowcount);
  end loop;
end;
