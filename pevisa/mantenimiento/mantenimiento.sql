...
drop table OT_MANTTO_DIARIO_BITACORA cascade constraints;

CREATE TABLE OT_MANTTO_DIARIO_BITACORA(
  NUMERO NUMBER NOT NULL,--PK
  ID_TIPO VARCHAR2(2) NOT NULL,--FK OT_MANTTO
  ID_SERIE NUMBER(3) NOT NULL,--FK OT_MANTTO
  ID_NUMERO NUMBER(6) NOT NULL,--FK OT_MANTTO
  HORA_INICIO DATE NOT NULL,
  HORA_FIN DATE,
  ID_PERSONAL VARCHAR2(8) NOT NULL,
  ID_ACTIVIDAD number(6) NOT NULL,--FK OT_ACTIVIDADES_MANTENIMIENTO
...
CREATE UNIQUE INDEX PEVISA.OT_BITACORA_PK ON PEVISA.OT_MANTTO_DIARIO_BITACORA   --indice para el primary key
  (NUMERO)
  LOGGING
  TABLESPACE PEVISAX
  PCTFREE    10
...
ALTER TABLE PEVISA.OT_MANTTO_DIARIO_BITACORA ADD (--define primary
  CONSTRAINT OT_MANTTO_D_B_PK
    PRIMARY KEY
      (NUMERO)
      USING INDEX PEVISA.OT_BITACORA_PK
      ENABLE VALIDATE);

alter table PEVISA.OT_MANTTO_DIARIO_BITACORA add (--define fk
  constraint FK_BITACORA_ID_SERIE
    foreign key (ID_TIPO,ID_SERIE,ID_NUMERO)
      references PEVISA.OT_MANTTO(ID_TIPO,ID_SERIE,ID_NUMERO)
        on delete cascade
        enable validate);

alter table PEVISA.OT_MANTTO_DIARIO_BITACORA add (--define fk
  constraint FK_BITACORA_ID_ACTIVIDAD
    foreign key (ID_ACTIVIDAD)
      references PEVISA.OT_ACTIVIDADES_MANTENIMIENTO(ID_ACTIVIDAD)
        on delete cascade
        enable validate);

CREATE OR REPLACE PUBLIC SYNONYM OT_MANTTO_DIARIO_BITACORA FOR PEVISA.OT_MANTTO_DIARIO_BITACORA;-- todos

select *
  from OT_MANTTO_DIARIO_BITACORA;

select * from OT_ACTIVIDADES_MANTENIMIENTO;
